...
SMOKE_CONSTRAINT="${SMOKE_CONSTRAINT:-local only}"
SMOKE_CRITERIA="${SMOKE_CRITERIA:-tests pass}"
...
RAW_INTENT="$(printf 'goal: %s\nscope: %s\nconstraints: %s\nsuccess_criteria: %s' "$SMOKE_GOAL" "$SMOKE_SCOPE" "$SMOKE_CONSTRAINT" "$SMOKE_CRITERIA")"
